...
/**
 * Kui me jookseme mitut backend-instantsi (k8s HPA), peab rate-limit state
 * olema jagatud — muidu saab user N × limit, kus N = pod count.
 *
 * <p>Sellepärast kasutame bucket4j-redis'e ProxyManager'it: bucket state
 * elab Redis'es, CAS (compare-and-swap) atomic'ute update'idega. Iga pod
 * näeb sama tõde.
 *
 * <p>Profile: {@code rate-limit-distributed} — alles siis, kui Redis on
 * reachable. Single-node'is (dev, CI) kasutame in-memory bucket'e edasi.
 */
@Configuration
...
public class DistributedBucketFactory {
    ...
    @Bean
    public RedisClient rateLimitRedisClient(@Value("${app.redis.url:redis://redis:6379}") String url) {
