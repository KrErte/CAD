...
/**
 * Talks to the Python slicer sidecar — turns an STL into estimated print time,
 * filament mass and EUR cost by shelling out to PrusaSlicer CLI.
 *
 * The sidecar is OPTIONAL: when SLICER_URL is blank the backend falls back to
 * the worker's volume-based heuristic (see {@link DesignController#preview}).
 */
@Component
public class SlicerClient {
    ...
    /**
     * Slices the given STL with the given preset and optional overrides.
     * Returns the JSON payload emitted by the sidecar (see slicer/app.py).
     */
