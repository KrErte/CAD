...
/**
 * Fan-out SSE-emitter pool. Iga org-i jaoks hoiame listi avatud emitter'ist;
 * iga printeri refresh → publish → kõik listenerid saavad pushi.
 */
@Component
public class PrinterEventPublisher {
    ...
    public synchronized SseEmitter register(Long orgId) {
        ...
        try { em.send(SseEmitter.event().name("connected").data(Map.of("ok", true))); }
