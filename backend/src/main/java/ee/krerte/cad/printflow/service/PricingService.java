...
/**
 * Arvutab quote-line hinna slicer-tulemusest + materjali hinnast + organisatsiooni hourly-rate'ist
 * + margin'ist.
 *
 * <p>Valem: base = filament_cost_eur + (print_time_sec / 3600) * hourly_rate unit = max(base,
 * setup_fee) * (1 + margin_pct/100) * rush_multiplier line_total = unit * quantity * (1 -
...
public class PricingService {
    ...
    public LinePricing calculate(
        ...
        BigDecimal filamentCost =
                material.getPricePerKgEur()
                        .multiply(BigDecimal.valueOf(filamentG))
                        .divide(BigDecimal.valueOf(1000), 4, RoundingMode.HALF_UP);

        // 2) masinahind
        BigDecimal hourlyRate =
                material.getOrganizationId() != null
                        ? org.getHourlyRateEur()
                        : new BigDecimal("2.50");
        BigDecimal timeCost =
                hourlyRate
        ...
        BigDecimal marginMult =
                BigDecimal.ONE.add(BigDecimal.valueOf(marginPct).divide(BigDecimal.valueOf(100)));
        ...
        p.totalEur = total;
        p.volumeDiscountPct =
                discount.movePointRight(2).setScale(0, RoundingMode.HALF_UP).intValue();
