 ...
 *
 * <p>Ainus "instant-quote" kood, mida kogu tööstus vajab.
 */
@Service
public class QuoteService {
    ...
    public QuoteService(
            QuoteRepository q,
            QuoteLineRepository l,
            MaterialRepository m,
            DfmService d,
            SlicerClient s,
            PricingService p,
            PrintJobRepository j,
            ObjectMapper om) {
        this.quoteRepo = q;
        this.lineRepo = l;
        this.materialRepo = m;
    ...
    public QuoteResult createFromUpload(
        ...
        Material material =
                materialRepo
                        .findByIdAndOrganizationId(materialId, org.getId())
                        .orElseThrow(
                                () ->
                                        new ResponseStatusException(
                                                HttpStatus.BAD_REQUEST, "materjal ei leitud"));
        ...
        PricingService.LinePricing pricing =
    ...
    private String nextQuoteNumber(Organization org) {
        ...
        return String.format(
                "Q-%d-%04d", Instant.now().atZone(java.time.ZoneOffset.UTC).getYear(), count + 1);
    }
    ...
    private Map<String, Object> sliceOrFallback(byte[] stl, Material material, Integer infillPct) {
        ...
        try {
            String preset =
                    material.getSlicerPreset() != null
                            ? material.getSlicerPreset()
                            : (material.getFamily() != null
                                            ? material.getFamily().toLowerCase()
                                            : "pla")
                                    + "_default";
            String fill = infillPct != null ? infillPct + "%" : "20%";
            JsonNode res = slicerClient.slice(stl, preset, fill, "0.20");
            out.put("source", "slicer");
            if (res.has("print_time_sec"))
                out.put("print_time_sec", res.get("print_time_sec").asLong());
            if (res.has("print_time_human"))
                out.put("print_time_human", res.get("print_time_human").asText());
            if (res.has("filament_g")) out.put("filament_g", res.get("filament_g").asDouble());
            if (res.has("filament_cost_eur"))
                out.put("filament_cost_eur", res.get("filament_cost_eur").asDouble());
            if (res.has("filament_volume_cm3"))
                out.put("filament_volume_cm3", res.get("filament_volume_cm3").asDouble());
        } catch (Exception e) {
    ...
    public Quote accept(Long quoteId, Long orgId) {
        Quote q =
                quoteRepo
                        .findByIdAndOrganizationId(quoteId, orgId)
                        .orElseThrow(
                                () ->
                                        new ResponseStatusException(
                                                HttpStatus.NOT_FOUND, "quote ei leitud"));
        if (!"DRAFT".equals(q.getStatus()) && !"SENT".equals(q.getStatus())) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "quote pole nõustutav staatuses " + q.getStatus());
        }
        ...
        for (QuoteLine line : lineRepo.findByQuoteIdOrderByLineNoAsc(q.getId())) {
            for (int i = 0; i < line.getQuantity(); i++) {
                ...
                job.setPriority(50);
                job.setJobName(
                        (line.getFileName() != null ? line.getFileName() : "part")
                                + " ("
                                + (i + 1)
                                + "/"
                                + line.getQuantity()
        ...
        log.info(
                "Quote {} aktsepteeritud, {} line'i → jobs",
                q.getQuoteNumber(),
                lineRepo.findByQuoteIdOrderByLineNoAsc(q.getId()).size());
        return q;
    }
    ...
    public Quote get(Long id, Long orgId) {
        return quoteRepo
                ...
                .orElseThrow(
                        () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "quote ei leitud"));
    }
