...
/**
 * Valib õige adapteri printeri `adapterType` järgi. V1-s on ainult MOCK,
 * aga signature on juba valmis tuleviku integratsioonide jaoks (BAMBU,
 * MOONRAKER, OCTOPRINT, PRUSA_CONNECT).
 */
@Component
public class PrinterAdapterFactory {
    ...
    public PrinterAdapterFactory(List<PrinterAdapter> adapters, MockPrinterAdapter mock) {
        this.byType = adapters.stream().collect(Collectors.toMap(PrinterAdapter::supportsType, a -> a));
        this.fallback = mock;
    }
