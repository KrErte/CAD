...
public class SpoolController {
    ...
    public List<Map<String, Object>> list() {
        ...
        return repo.findByOrganizationIdOrderByCreatedAtDesc(org.getId()).stream()
                .map(SpoolController::render)
                .toList();
    }

    @GetMapping("/low-stock")
    public List<Map<String, Object>> lowStock(
            @RequestParam(value = "threshold_g", defaultValue = "100") int thresholdG) {
        Organization org = orgCtx.currentOrganization();
        return repo.findLowStock(org.getId(), thresholdG).stream()
                .map(SpoolController::render)
                .toList();
    }
    ...
    public Map<String, Object> update(@PathVariable Long id, @RequestBody FilamentSpool patch) {
        ...
        if (patch.getAssignedPrinterId() != null)
            s.setAssignedPrinterId(patch.getAssignedPrinterId());
    ...
    public static Map<String, Object> render(FilamentSpool s) {
        ...
        m.put(
                "pct_remaining",
                s.getMassInitialG() != null && s.getMassInitialG() > 0
                        ? Math.round((double) s.getMassRemainingG() / s.getMassInitialG() * 100)
                        : 0);
