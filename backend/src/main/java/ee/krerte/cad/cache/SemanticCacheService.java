 ...
 *
 * <p>Idee: kui kasutaja kirjutab "20mm kuup", siis 10min hiljem keegi teine kirjutab "kuup 20mm
 * suurune", on vastus sisuliselt sama. Claude maksab ~0.01€ per spec, ja kui meil on 10k kasutajat,
 * säästame embedding-lookup'iga ~90% korduvatest kõnedest.
 *
 * <p>Algoritm:
 *
 * <ol>
 *   <li>Hash prompti (exact match kõige kiirem) — kui leiame, tagastame kohe
 *   <li>Embeddi prompt 1536-dim vektoriks (Voyage / OpenAI)
 *   <li>Otsi pgvector'ist lähim prompt, cosine distance &lt; 0.05 = samasugune
 *   <li>Kui ei leia, kutsume Claude'i ja salvestame uue embedding'u
 * </ol>
 *
 * <p>See klass ei tee ise embedding-API kõnet — {@link EmbeddingClient} (eraldi) teeb. Siin on
 * ainult lookup + store Postgres'is.
 */
@Service
    ...
    public Optional<JsonNode> lookup(String prompt, float[] embedding) {
        ...
        try {
            ...
            if (distance < SIMILARITY_THRESHOLD) {
                jdbc.update(
                        "UPDATE prompt_embeddings SET hit_count = hit_count + 1, last_hit_at = NOW() WHERE id = ?",
                        row.get("id"));
