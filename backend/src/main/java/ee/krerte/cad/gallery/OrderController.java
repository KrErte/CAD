...
/**
 * Print order & instant quote system.
 * Pricing: material cost + print time cost + shipping.
 */
@RestController
...
public class OrderController {
    ...
    private static final Map<String, Double> SHIPPING = Map.of(
        "EE", 3.99, "LV", 5.99, "LT", 5.99, "FI", 7.99, "SE", 9.99, "DE", 9.99
    );
    ...
    public record QuoteRequest(double weightG, int printTimeMin, String material,
                               int infillPct, int quantity, String country) {}
    ...
    public ResponseEntity<?> quote(@RequestBody QuoteRequest req) {
        ...
        double shipping = SHIPPING.getOrDefault(
                req.country() != null ? req.country().toUpperCase() : "EE", SHIPPING_DEFAULT);
        ...
        quote.put("estimated_days", req.country() != null && req.country().equalsIgnoreCase("EE") ? "2-3" : "5-7");
        return ResponseEntity.ok(quote);
    }

    /** Place a print order. */
    public record PlaceOrderRequest(Long designId, String material, int infillPct,
                                    int quantity, String color, String shippingName,
                                    String shippingAddress, String shippingCity,
                                    String shippingZip, String shippingCountry, String notes) {}
    ...
    public ResponseEntity<?> placeOrder(@RequestBody PlaceOrderRequest req) {
        ...
        double shipping = SHIPPING.getOrDefault(
                req.shippingCountry() != null ? req.shippingCountry().toUpperCase() : "EE", SHIPPING_DEFAULT);
        ...
        return ResponseEntity.ok(Map.of(
                ...
                "price_eur", order.getPriceEur(),
                "status", "pending",
                "message", "Tellimus vastu võetud! Saadame kinnituse meilile."));
    }

    /** List my orders. */
    @GetMapping
    public List<Map<String, Object>> myOrders() {
        return orderRepo.findByUserIdOrderByCreatedAtDesc(uid()).stream().map(o -> {
