...
/**
 * Readiness health check — CadQuery worker sidecar. k8s readiness probe annab pod'i Service'ist
 * välja kui see UP → DOWN liigub.
 */
@Component("worker")
public class WorkerHealthIndicator implements HealthIndicator {
    ...
    public WorkerHealthIndicator(
            @Value("${app.worker.url:http://localhost:8000}") String workerUrl) {
