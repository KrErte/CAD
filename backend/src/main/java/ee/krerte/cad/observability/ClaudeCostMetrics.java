 ...
 *
 * <p>Igal Claude kutsumisel ({@code ClaudeClient.call(...)} jne) peaks kutsuma {@link
 * #recordUsage(String, long, long, long)} — see registreerib:
 *
 * <ul>
 *   <li>{@code claude.api.tokens.input} — counter per mudel
 *   <li>{@code claude.api.tokens.output} — counter per mudel
 *   <li>{@code claude.api.duration} — timer (histogram p50/p95/p99)
 *   <li>{@code claude.api.cost.eur} — counter (€ summa, arvutatud input/output token count'ist *
 *       app.pricing-claude.*)
 *   <li>{@code claude.api.requests.total} — counter per mudel + staatus
 * </ul>
 *
 * Prometheus scrape'ib need, Grafana dashboard näitab: "Claude € täna" =
 * increase(claude_api_cost_eur_total[24h]) "Input token/s" =
 * rate(claude_api_tokens_input_total[5m])
 */
@Component
public class ClaudeCostMetrics {
    ...
    public void recordUsage(String model, long inputTokens, long outputTokens, long durationMs) {
        ...
        double costEur =
                (inputTokens / 1_000_000.0) * inputEurPerMtok
        ...
        registry.counter("claude.api.requests.total", "model", model, "status", "success")
                .increment();
    }
