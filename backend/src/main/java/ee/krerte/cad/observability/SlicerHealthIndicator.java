...
/**
 * Readiness health check — PrusaSlicer sidecar (optional).
 * Kui app.slicer.url on tühi (dev-setup), märgime UP "disabled" meldinguga —
 * mitte DOWN, kuna slicer on vabatahtlik komponent.
 */
@Component("slicer")
public class SlicerHealthIndicator implements HealthIndicator {
    ...
    public SlicerHealthIndicator(@Value("${app.slicer.url:}") String slicerUrl) {
        ...
        this.client = (slicerUrl == null || slicerUrl.isBlank())
                ? null
