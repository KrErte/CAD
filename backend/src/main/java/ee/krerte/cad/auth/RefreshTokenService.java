 ...
 * <h3>Miks?</h3>
 *
 * Praegu tagastame 24h eluga JWT. Kui see JWT lekib, ründaja saab kuni 24 tundi käia. Kui me
 * kasutame 15min JWT + pika refresh-token'i, siis ründaja kaotab access'i 15 min pärast lekket.
 *
 * <h3>Rotation + family</h3>
 *
 * Iga refresh tagastab UUS refresh token, vana invalidate'itakse (replaced_by_id). Kui ründaja ja
 * ohver kasutavad sama VANA tokenit, siis me näeme reuse → revoke kogu family → ohver peab uuesti
 * login'ima, ründaja ei pääse enam.
 *
 * <h3>Storage</h3>
 *
 * DB'sse salvestame token'i SHA-256 hash, mitte raw. DB leak ei anna ründajale kasutatavaid
 * tokeneid.
 */
@Service
public class RefreshTokenService {
    ...
    /**
     * Rotate — kontrollib et token on kehtiv, märgib vana kasutatuks, annab välja uue. Kui keegi
     * proovib sama tokenit uuesti kasutada, siis revoke kogu family.
     *
    ...
    public Optional<RotationResult> rotate(String rawToken, HttpServletRequest req) {
        ...
        Instant revokedAt =
                ((java.sql.Timestamp) row.getOrDefault("revoked_at", null)) == null
                        ? null
                        : ((java.sql.Timestamp) row.get("revoked_at")).toInstant();
        ...
        if (revokedAt != null) {
            log.warn(
                    "Refresh token REUSE detected — revoking family user={} family={}",
                    userId,
                    familyId);
            jdbc.update(
    ...
    private static String sha256(String s) {
        try {
            return HexFormat.of()
                    .formatHex(MessageDigest.getInstance("SHA-256").digest(s.getBytes()));
        } catch (Exception e) {
