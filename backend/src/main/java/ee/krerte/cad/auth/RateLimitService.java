...
/**
 * Per-user per-bucket sliding-window rate limiter. In-memory, no Redis.
 * Good enough for single-instance deploy; swap to Redis when scaling horizontally.
 *
 * Buckets:
 *   - "spec"     : limits calls to /api/spec (Claude API, expensive)
 *   - "generate" : limits STL generation
 */
@Service
public class RateLimitService {
    ...
    public RateLimitService(@Value("${app.ratelimit.spec-per-hour:20}") int specPerHour,
