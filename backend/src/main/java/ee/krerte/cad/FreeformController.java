...
/**
 * Freeform CadQuery sandbox — lubab Pro+ kasutajal kirjutada oma CadQuery koodi ja see jookseb
 * turvalises worker-sandbox'is (AST-whitelist, 15s timeout, 512MB memory limit).
 *
 * <p>Ainult PRO ja TEAM plaanidel — Free ja Maker näevad 403 "upgrade required". See on meie
 * Pro-plaani peamine eristaja: 23 malli pole piisav → kirjuta ise.
 *
 * <p>Worker tagastab <code>{ok, error, error_kind, files:{stl,step}, elapsed_ms}</code> ja me
 * edastame selle frontendile muutmata kujul. STL/STEP on base64-kodeeritud stringi­dena JSON-is, et
 * front saaks kohe data-url-iga alla laadida.
 */
@RestController
            ...
            return ResponseEntity.status(403)
                    .body(
                            Map.of(
                                    "ok", false,
                                    "error",
                                            "Freeform sandbox on saadaval ainult Pro ja Team plaanidel. Uuenda plaani, et jätkata.",
            ...
            return ResponseEntity.status(502)
                    .body(
                            Map.of(
                                    ...
                                    false,
                                    "error",
                                    e.getMessage() == null ? "Worker error" : e.getMessage(),
                                    "error_kind",
                                    "worker_unreachable"));
        }
