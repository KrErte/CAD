...
/**
 * Contract test ClaudeClient'ile — tagame, et me saadame Anthropic Messages API'le õige kujuga
 * päringu (model, system, messages, max_tokens) ja suudame parsida nii puhast JSON'it kui ka
 * ```json fenced vastust.
 *
 * <p>Kasutame WireMock'i, et:
 *
 * <ul>
 *   <li>Ei kuluta päris API-krediiti (~0.01€ per kõne × sadu teste = päris raha)
 *   <li>Determinismi: sama päring = sama vastus iga kord
 *   <li>Kontraktid: kui Anthropic muudaks response schema't, siis meie kood katkeks siit, mitte
 *       prod'is
 * </ul>
 */
@SpringBootTest
...
class ClaudeClientContractTest {
    ...
    void happyPath_cleanJson() throws Exception {
        ...
        JsonNode catalog =
                json.readTree("{\"cube\":{\"size\":{\"min\":10,\"max\":100,\"default\":20}}}");
