...
/**
 * Kontrollime live-serveril, et security headers on kohal. Kui keegi tulevikus SecurityConfig'u
 * katki kaoab (või CSP'i kogemata eemaldab), see test lööb alarmi enne, kui prod'is saaks XSS-i
 * lasta.
 */
...
class SecurityHeadersIT extends AbstractPostgresIntegrationTest {
    ...
    void headersPresent() {
        ...
        assertThat(headers.getFirst("X-Frame-Options")).as("Clickjacking-kaitse").isEqualTo("DENY");

        assertThat(headers.getFirst("X-Content-Type-Options"))
                .as("MIME-sniff kaitse")
                .isEqualTo("nosniff");
